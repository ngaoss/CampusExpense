...
public class ExpenseDao {
    ...
    public List<RecurringExpense> getAllRecurringRules() {
    ...
    public List<Expense> getOneTimeExpensesForMonth(int year, int month) {
        ...
        try (Cursor cursor = db.rawQuery(query, new String[]{yearMonthStr})) {
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    int idIndex = cursor.getColumnIndex(DatabaseHelper.KEY_ID);
                    int amountIndex = cursor.getColumnIndex(DatabaseHelper.KEY_AMOUNT);
                    ...
                    Category category = new Category(cursor.getLong(catIdIndex), cursor.getString(catNameIndex));
                    oneTimeExpenses.add(new Expense(
                            cursor.getLong(idIndex),
                            cursor.getDouble(amountIndex),
                            cursor.getString(descIndex),
                            cursor.getString(dateIndex),
    ...
    public List<Expense> getExpensesForMonth(int year, int month, String recurringPrefix) {
        ...
        String oneTimeQuery = "SELECT E.*, C." + DatabaseHelper.KEY_CATEGORY_NAME + " as cat_name, C." + DatabaseHelper.KEY_ID + " as cat_id"
                + " FROM " + DatabaseHelper.TABLE_EXPENSES + " E JOIN " + DatabaseHelper.TABLE_CATEGORIES
                + " C ON E." + DatabaseHelper.KEY_CATEGORY_ID + " = C." + DatabaseHelper.KEY_ID
                + " WHERE strftime('%Y-%m', E." + DatabaseHelper.KEY_EXPENSE_DATE + ") = ?";

        try (Cursor cursor = db.rawQuery(oneTimeQuery, new String[]{yearMonthStr})) {
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    int idIndex = cursor.getColumnIndex(DatabaseHelper.KEY_ID);
                    int amountIndex = cursor.getColumnIndex(DatabaseHelper.KEY_AMOUNT);
        ...
        Calendar queryMonthCalendar = Calendar.getInstance();
        queryMonthCalendar.set(year, month - 1, 1);
        for (RecurringExpense rule : allRecurringRules) {
            try {
                ...
                if (ruleStartDate.after(queryMonthCalendar) && ruleStartDate.get(Calendar.MONTH) != (month-1)) {
                    continue;
                }

                double projectedAmount = 0;
                String projectedDescription = recurringPrefix + " " + rule.getDescription();
                String representativeDate = yearMonthStr + "-01";
                if ("MONTHLY".equalsIgnoreCase(rule.getFrequency())) {
                    projectedAmount = rule.getAmount();
                } else if ("DAILY".equalsIgnoreCase(rule.getFrequency())) {
                    int daysInMonth = queryMonthCalendar.getActualMaximum(Calendar.DAY_OF_MONTH);
                    projectedAmount = rule.getAmount() * daysInMonth;
                    projectedDescription += " (Monthly Total)";
                }
