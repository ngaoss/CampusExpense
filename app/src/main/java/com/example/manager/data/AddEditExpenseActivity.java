...
public class AddEditExpenseActivity extends AppCompatActivity {
    ...
    private void checkBudgetAndNotify(Category category) {
        ...
        if (budgetLimit > 0) {
            double oneTimeSpending = expenseRepo.getTotalExpensesForCategory(category.getId());
            double recurringSpending = recurringRepo.getEstimatedMonthlyRecurringForCategory(category.getId());
